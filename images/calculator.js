// SELECT ELEMENTS
const input_elements = document.querySelector('.input')
...
// CREATE CALCILATOR BUTTONS
function createCalculationButtons () {
    const btns_per_row = 8;
    let added_btns = 0;
    calculator_buttons.forEach( button => {
        if(added_btns % btns_per_row == 0) {
            input_elements.innerHTML += `<div class="row"></div>`;
        }
...
function factorialNumberGetter(formula, FACTORIAL_SEARCH_RESULT) {
    ...
    FACTORIAL_SEARCH_RESULT.forEach( factorial_index => {
        let numbers = []; // current factorial number
        ...
        while ( previous_index >= 0 ) {
            ...
            numbers.unshift( formula[previous_index] );
            previous_index--;
        }
...
function powerBaseGetter(formula, POWER_SEARCH_RESULT) {
    ...
    POWER_SEARCH_RESULT.forEach( power_index => {
        ...
        while ( previous_index >= 0 ) {
            ...
            index.unshift( formula[previous_index] );
            previous_index--;
        }
